...
write.table(data, "./Myofibroblasts_metastasis_primary_DEG.txt", col.names=T, row.names=T, quote=F, sep="\t")
...
write.table(KEGG_ges@result, "./Myofibroblasts_GSEA_result.txt", col.names=T, row.names=T, quote=F, sep="\t")
#################################################################################
# 画带有基因标签的GSEA分析图
KEGG_ges2 <- setReadable(KEGG_ges, OrgDb=org.Hs.eg.db, keyType="SYMBOL")
core <- result$core_enrichment[7]
core_genes <- str_split(core ,'/')[[1]]
p = gseaNb(
    object = KEGG_ges,
    geneSetID = result$ID[7],
    addPval = T,
    pvalX = 0.95,
    pvalY = 0.8,
    newGsea = T,
...
ggsave("Myofibroblasts_GSEA_Antigen_processing.pdf", p, width=6, height=6)
##########################################################################
# 画两组间GSEA分析的NES得分的棒棒糖图
library(ggplot2)
df = read.table("./Myofibroblasts_GSEA_result_barplot.txt", header=T, sep="\t")
